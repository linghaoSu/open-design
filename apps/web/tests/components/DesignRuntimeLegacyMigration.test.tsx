...
describe('legacy migration panel', () => {
  it('reviews canonical legacy files without JSON authoring, shows incomplete facts and applies the exact proof once', async () => {
    ...
    expect(element('legacy-token-stylesheet').value).toBe('tokens.css'); expect(element('legacy-apply').disabled).toBe(true);
    await review();
    ...
    expect(element('legacy-apply').disabled).toBe(true);
  });
  it('offers system/variables.css and optional editable React/Vue selections including removal of the last component', async () => {
    ...
    fireEvent.click(element('legacy-add-component')); fireEvent.change(element('design-runtime-framework-0'), { target: { value: 'vue' } });
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Delete 1' })); expect(screen.queryByTestId('design-runtime-export-name-0')).toBeNull(); expect(element('legacy-apply').disabled).toBe(true);
  });
  it.each(['version', 'name', 'mode', 'files', 'revision'] as const)('invalidates an existing review when %s changes', async (change) => {
    ...
    expect(element('legacy-apply').disabled).toBe(true); expect(screen.queryByTestId('legacy-review-result')).toBeNull();
  });
  it.each(['workspace', 'source'] as const)('aborts pending review on %s changes and ignores its late response', async (change) => {
    ...
    expect(screen.queryByTestId('legacy-review-result')).toBeNull(); expect(element('legacy-apply').disabled).toBe(true);
  });
  it('allows a readonly review while preventing application and reports a blocked document-only candidate', async () => {
    ...
    expect(element('legacy-apply').disabled).toBe(true); expect(screen.getByText('No convertible facts.')).toBeTruthy(); expect(provider.applyProjectDesignRuntimeLegacyMigration).not.toHaveBeenCalled();
  });
  it('does not adopt a late application after the Workspace identity changes', async () => {
    ...
    expect(input.onState).not.toHaveBeenCalled(); expect(screen.queryByTestId('legacy-review-result')).toBeNull(); expect(element('legacy-apply').disabled).toBe(true);
  });
  it('invalidates a stale source proof, refreshes the project, and never retries apply', async () => {
    ...
    await waitFor(() => expect(input.onState).toHaveBeenCalledOnce()); expect(element('legacy-apply').disabled).toBe(true); expect(provider.applyProjectDesignRuntimeLegacyMigration).toHaveBeenCalledOnce();
  });
});
