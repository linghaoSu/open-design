...
export const DesignSystemMigrationPlanSchema = z.object({
  ...
  plan.rules.forEach((entry, index) => {
    ...
    if (!ref.startsWith(`ds:${plan.from.designSystemId}/`) || (entry.type === 'replace-component' && !entry.toRef.startsWith(`ds:${plan.to.designSystemId}/`))) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['rules', index], message: 'Rule references must belong to the selected design system.' });
    if (target !== undefined) {
