...
当前结论：`f6c5911c5` 已从 `/Applications/Design Loom.app` 冷启动，并完成概览、分类明细、版本解除与重新锁定、设置状态、文件链接及真实主题组件预览的原生操作。后续发现的属性回显、历史失败归属、上游问卷及隐私说明问题，均已在真实应用中完成设计审阅后实施，并通过源码回归。新修复仍需重新构建、安装与原生验收，尚不标记整体验收完成。独立应用未启用上游更新；此包**仅具 ad hoc 签名，未经 Developer ID 签名与公证**，尚未满足公开分发门槛。
...
- [ ] **原生验收新增问题收敛**：属性未声明值回显、历史任务错误归属、上游问卷及隐私说明的设计、实施和源码回归已完成；仍需构建、安装并实际操作新包。已有候选的原生通过记录不自动转用于新包。
