...
This independent fork ships **Design Loom** through the macOS path, using the product identity and namespace restrictions in the root guide. The inherited Windows/Linux and upstream channel/updater harness descriptions below are architecture references, not active delivery instructions for this fork. Design Loom disables upstream updates and telemetry, uses its own app bundle and protocol, and does not install a global `od` shim. See [the Design Loom guide](../../docs/design-loom.md) for current build and acceptance commands.
...
- Post-update "what's new" highlights are NOT carried in release `metadata.json`. This fork has no default upstream highlights feed. The daemon's `/api/whats-new` can read an explicitly configured `OD_WHATS_NEW_URL` during development; the Design Loom packaged launcher clears that override. When configured, the web card uses the document's `id`, not the running version. There is no per-version highlights publish tooling.
